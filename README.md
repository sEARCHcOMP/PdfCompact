...
> **Version 4.1.1** · 2026-07-06
> ブラウザ完結型・サーバー不要・100% LOCAL の PDF ツールキット

[![download](https://img.shields.io/badge/download-ZIP%20(4.1.1)-1a85ff?style=for-the-badge)](https://pdfcompact.pages.dev/PDF_Compact.zip)
...
| 🖤 **黒塗り(墨消し)** | 黒/背景色を選んで文字データごと物理消去する本物のリダクション、出力前に自動検証 |
...
### v4.1.1 (2026-07-06) — Patch (📖 文言の手直し)
- 「背景色で消す」(v4.1.0)に合わせて画面・取説の文言を整理: 黒を前提にした説明(「黒塗りが付きます」「黒塗りの無いページは〜」等)を「塗りつぶし」に統一。道具の名前としての「黒塗り(墨消し)」は従来どおり
- **背景色モードの出力ファイル名を「修正_日付.pdf」に変更**(黒は従来どおり「墨消し_日付.pdf」)。背景色モードは「消した痕を見せない」のが目的のため、ファイル名でも痕跡を出さない
- 処理内容の変更なし(変更は文言と出力ファイル名のみ)

### v4.1.0 (2026-07-06) — Minor (⬜ 黒塗りに「背景色で消す」を追加)
- **塗りつぶしの色を選択可能に**: 従来の「黒で塗る」に加え「背景色で消す」を追加。矩形の周囲8pxリングをサンプリングし各チャンネル中央値で紙の色を推察(黄ばんだスキャンにも馴染む)。他の黒塗り矩形の内側はサンプルから除外、標本不足時は白にフォールバック
- 色の確定は全矩形ぶん塗る前に一括実施(先に塗ると隣接矩形のサンプリングが汚れるため)
- 仮表示(編集画面)も色モードに追随(背景色モードは白ベース+破線)。出力ボタンのラベルも切替
- **物理消去の仕組み・出力前の自動検証は色に関係なく同一**(canvas 焼き込みのみ、偽リダクション構造は作らない)
- テスト+1本(黄ばんだ紙で「黒くない・真っ白でもない・周囲の色に馴染む」をピクセル検証)
