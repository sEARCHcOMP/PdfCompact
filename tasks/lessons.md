...
## リファクタリング着手前に必読 (2026-06-10)

### 11. 大規模リファクタの前に `docs/plans/refactoring-handoff.md` を読め

v3.7.4〜3.7.6 + 黒塗り11修正 + 監査(39バグ撃破)を通った Fable 5 が、リファクタの**設計思想・地雷の位置・検証作法・過去の失敗**を全部書き残した引き継ぎノート。会話の記憶なしでも続行できるように書いてある。リファクタ(Phase 0〜5)に着手するなら、ロードマップ(`refactoring-roadmap.md`)より先にこのノートを読むこと。

**特に効く要点**:
- このコードベースのバグの1/3は「共通部品が無いから同じ間違いをN回する」構造的なもの。座標写像が imgPlace と黒塗りで2実装ある等。リファクタ=バグ予防策。
- **順序の絶対則**: ファイル分割すると feat/redaction が永久にマージ不能になる。だから「黒塗りを v3.8.0 で main 着地」→「テスト基盤」→ようやく分割。この順序だけは死守。
- 検証は「ピクセルかバイトで機械判定できるまで」やる。表面の結果を信じるな(透かしを秘密残骸と誤認・blob detachで空判定など、今日3回偽の結果に騙された)。
- プレビューが重いページで劣化したら `location.reload()` では不十分、**preview_stop→start でブラウザごと再起動**。
- 単一HTML・依存ゼロ追加・bundler不採用は「好み」やなく製品を動かすための物理法則。
